...
- **Complete Documentation, UI Screenshot & Vector PDF Manual Refresh (v9.15.1):** Refreshed all 22 high-resolution UI screenshots across all views to showcase primary reference hardware (RAEDIAN NEO smart wallbox and Enerlution L3–L12 hybrid inverter/BESS), reflecting system release `v9.15.1-release`. Synchronized all 5 official printable vector PDF manuals (`gems-installer-manual.pdf`, `gems-user-manual.pdf`, `gems-server-connection-manual.pdf`, `gems-p1-meter-manual.pdf`, and `gems-supported-hardware-manual.pdf`), documentation hub pages, and README with the full 140-template hardware matrix across all 5 device classes.
...
### Fixed
- **Dynamic Load Balancing Hardware Command Dispatch:** Resolved an issue in fleet charging mode where dynamic load balancing (DLB) setpoints were recorded in memory but bypassed physical command dispatch to connected hardware controllers. Setpoints are now reliably transmitted to all physical wallboxes while maintaining seamless synchronization with external OCPP endpoints.
- **Dynamic Forecast Battery Arbitrage Profitability:** Updated the predictive battery schedule calculation to strictly account for 85% round-trip conversion efficiency losses and configured battery cell degradation wear costs before scheduling grid discharge, ensuring battery energy is only discharged when wholesale price spreads guarantee net financial profit.
- **EV Charger Configured Maximum Current Scaling:** Replaced fixed 16A limits in EV charging intent logic, solar excess sinking, Flanders ramp-up, and Netherlands export absorption with the device's configured maximum rated current (up to 32A), allowing high-capacity chargers to charge at their full rated speed.
- **Flanders Peak Shaving Control Loop Smoothing:** Clamped the quarter-hour peak excess amplification factor to 10x, eliminating extreme over-correction and sudden setpoint drops in the final seconds of each 15-minute capacity tariff window.
- **Dynamic Tariff Date Rollover Cache Invalidation:** Enhanced the strategy pricing cache to detect midnight day rollovers in addition to hourly boundaries, ensuring energy prices immediately refresh at 00:00 without delay.
- **Timezone Consistency Across Strategy Engine:** Standardized timezone handling across all scheduling routines to consistently use the installation's configured site timezone.
- **Solar Forecasting Timezone Precision:** Explicitly aligned Open-Meteo solar forecast requests with UTC time coordinates, ensuring hourly solar generation projections precisely match local time curves.
- **Elia National Wind Generation Summation:** Refined the Belgian transmission grid wind generation dataset ingestion to group records by timestamp, preventing duplicate segment summation when generation records update asynchronously.
- **Engie Flextime Weekend Super-Off-Peak Tariff:** Applied super-off-peak (super-dal, 01:00–07:00) markups to weekends for Engie Flextime and Drive contracts, ensuring accurate energy cost calculations 7 days a week.
- **Database Startup Integrity Verification:** Added automated SQLite integrity validation on startup to detect filesystem anomalies early.
- **Notification Webhook Rate Limiting & Deduplication:** Added a 60-second cooldown per distinct alert message in the webhook notification system, preventing notification floods during intermittent network or hardware events.
- **Event Bus Telemetry Reliability:** Expanded internal state broadcast channel buffers to prevent telemetry packet drops during momentary consumer latency.
